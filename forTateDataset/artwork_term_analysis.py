...
readDataPath = './data/artworks_tmp/artworks_doc_term'
statsWritePath = './data/artworks_stats'
figWritePath = './data/artworks_figs/doc_TermFigs'
splitsMergesWritePath = './data/splitsMerges/doc_Term'
if not os.path.exists(figWritePath):
    os.makedirs(figWritePath)

LVLs = ['lvl1','lvl2','lvl3','lvlA'] #'lvl1','lvl2','lvl3',
yearPeriods = ['2000s'] #'1800s',
trueYearsIni = [1964]#1800,
...
for idy,years in enumerate(yearPeriods):
    ...
    for lIdx,lvl in enumerate(LVLs):
        ...
        dataDict = {'uniquePersistentTerms':persistTerms}
        for pIdx in range(10):
            ...
            df = pd.read_table(readDataPath+'/binMat'+years+lvl+'_'+periodIdx+'.tsv', sep="\t", header=0,index_col=0)
            df = df.transpose()
            dfmax = df.max()
            dfmax[dfmax == 0] = 1
            df = df / dfmax
            ...
            interactive(True)
            plt.show()            
            fig.savefig(figWritePath+'/'+clusterAlgLabel+'Clusters/'+SOMdimensionsString+'/SOM_binMat'+years+lvl+'_'+periodIdx+'.png',bbox_inches='tight')
            plt.close()
            interactive(False)
            #-----------------------------------------------------------------------------------------------   
            #-------------------------------------------------------------------------------------------
            ...
            # interactive(True)
            # plt.show()      
            # plt.savefig(figWritePath+'/hierarchicalClustering/dendroCategory'+years+lvl+'_'+periodIdx+'.pdf',bbox_inches='tight')
            # plt.close()
            # interactive(False)
            ...
            # interactive(True)
            # plt.show()      
            # plt.savefig(figWritePath+'/hierarchicalClustering/dendroArtwork'+years+lvl+'_'+periodIdx+'.pdf',bbox_inches='tight')
            # plt.close()
            # interactive(False)
            ...
            # interactive(True)
            # fig.show()
            # fig.savefig(figWritePath+'/hierarchicalClustering/dendroHeatmap'+years+lvl+'_'+periodIdx+'.pdf',bbox_inches='tight')
            # plt.close()
            # interactive(False)
